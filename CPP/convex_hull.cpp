...
int main()
{
    ...
    for (int i = 5; i < num_of_itrations; i++){
        ...
        convexHull(points, sizeof(points)/sizeof(Point));
        ...
        run_time[count_] = std::chrono::duration_cast < std::chrono::nanoseconds > (stop - start).count();

        memory[count_] = physicalMemoryUsed / (1024.0 * 1024.0);
        ...
        cpuUsage = CalculateCPULoad(idleTime, kernelTime, userTime);
        cpu[count_] = cpuUsage;
    ...
    f_memory.open("CPP_peakMemoryUsage_ConvexHull.csv", ios::out | ios::app);
    f_run_time.open("CPP_runtimes_ConvexHull.csv", ios::out | ios::app);
    f_cpu.open("CPP_cpuUtilization_ConvexHull.csv", ios::out | ios::app);
